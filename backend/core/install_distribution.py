...
Invoke-WebRequest -Uri "{server_url}/downloads/install-proxy.ps1" -OutFile "install-proxy.ps1"
./install-proxy.ps1 -ProxyId "{proxy_id}" -Role {role} -Server "{server_url}" -Token "{install_token}" -Name "{name}"'''
