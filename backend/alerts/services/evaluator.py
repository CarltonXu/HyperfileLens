...
from alerts.choices import AlertStatus
...
def fire_alert(policy, resource=None, title="", message="", current_value=None, alert_key="default", metadata=None):
    ...
    if alert:
        ...
        alert.save(update_fields=["status", "last_triggered_at", "current_value", "message", "metadata", "updated_at"])
        return alert
    ...
    alert = AlertRecord.objects.create(
        ...
        unit=(policy.trigger_rule or {}).get("unit"),
        fingerprint=fingerprint,
