...
<template>
 <div v-if="totalItems > 0" class="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 bg-card border-t border-border">
 <!-- 显示信息 -->
 <div class="text-sm text-foreground-secondary">
 {{ t('pagination.showing') }} <span class="font-medium text-foreground">{{ startItem }}</span>
 {{ t('pagination.to') }} <span class="font-medium text-foreground">{{ endItem }}</span>
 {{ t('pagination.of') }} <span class="font-medium text-foreground">{{ totalItems }}</span>
 {{ t('pagination.items') }}
 </div>
 ...
 <span class="text-sm text-foreground-secondary">{{ t('pagination.pageSize') }}</span>
 <select
 :value="pageSize"
