...
<template>
 ...
 <h1 class="text-xl font-bold text-foreground tracking-tight">HyperFileLens</h1>
 <p class="text-sm text-foreground-secondary mt-1">{{ t('auth.loginSubtitle') }}</p>
 </div>
 ...
 <!-- Password -->
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.password') }}</label>
 <div class="relative">
 <div class="absolute left-3 top-1/2 -translate-y-1/2 text-foreground-muted">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <div class="relative h-9 w-24 rounded-lg border border-border bg-background-secondary cursor-pointer overflow-hidden hover:border-blue-500/50 transition-colors" @click="refreshCaptcha" :title="t('auth.refreshCaptcha')">
 <img
 v-if="captchaUrl"
 :src="captchaUrl"
 alt="Captcha"
 ...
 <input v-model="rememberMe" type="checkbox" class="w-3.5 h-3.5 rounded border-border-secondary bg-background text-blue-500 focus:ring-blue-500/50" />
 <span class="text-foreground-secondary">{{ t('auth.rememberMe') }}</span>
 </label>
 ...
 <span class="px-2 bg-card/90 text-foreground-muted">{{ t('auth.or') }}</span>
 </div>
 </div>
 ...
 <div class="modal-surface border border-border rounded-2xl shadow-2xl p-5 w-full max-w-xs">
 ...
 <h3 class="text-lg font-medium text-foreground">{{ t('auth.mfaRequired') }}</h3>
 <p class="text-sm text-foreground-secondary mt-1">{{ t('auth.mfaCodeSent') }}</p>
 </div>
