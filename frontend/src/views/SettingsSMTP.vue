  ...
  <div class="space-y-6">
      ...
      <table v-else class="min-w-full divide-y divide-border">
        <thead class="bg-background/50">
          <tr>
            ...
            <th
              class="px-6 py-3 text-right text-xs font-medium text-foreground-secondary uppercase tracking-wider">
              {{ t("common.actions") }}
            </th>
          </tr>
        </thead>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center">
                ...
                <span
                  ...
                  class="ml-2 px-2 py-0.5 text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300 rounded">
                  {{ t("common.default") }}
                </span>
              </div>
            </td>
            <td
              class="px-6 py-4 whitespace-nowrap text-sm text-foreground-secondary">
              {{ config.host }}:{{ config.port }}
              <span v-if="config.use_tls" class="ml-1 text-green-600"
                >(TLS)</span
              >
              <span v-else-if="config.use_ssl" class="ml-1 text-green-600"
                >(SSL)</span
              >
            </td>
            <td
    ...
    <div v-if="showDialog" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex min-h-screen items-center justify-center p-4">
        ...
        <div
          class="relative modal-surface rounded-lg shadow-xl max-w-lg w-full p-6">
          <h3 class="text-lg font-medium text-foreground mb-4">
            {{
              editing
                ? t("settings.smtp.editConfig")
                : t("settings.smtp.addConfig")
            }}
          </h3>
          ...
          <form @submit.prevent="saveConfig" class="space-y-4">
            <div>
              <label
                ...
                >{{ t("settings.smtp.name") }}</label
              >
              <input
                v-model="form.name"
                type="text"
                required
                class="mt-1 block w-full rounded-md border-border shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-3 py-2 border" />
            </div>
            ...
            <div class="flex items-center space-x-6">
              ...
              <label class="flex items-center">
                <input
                  ...
                  type="checkbox"
                  class="rounded border-border text-indigo-600 focus:ring-indigo-500" />
                <span class="ml-2 text-sm text-foreground-secondary">{{
                  t("common.active")
                }}</span>
              </label>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label
                  class="block text-sm font-medium text-foreground-secondary"
                  >{{ t("settings.smtp.fromEmail") }}</label
                >
                <input
              ...
              <div>
                <label
                  class="block text-sm font-medium text-foreground-secondary"
                  >{{ t("settings.smtp.fromName") }}</label
                >
                <input
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t">
              <button
                type="button"
                @click="closeDialog"
                class="px-4 py-2 text-sm font-medium text-foreground-secondary bg-background border border-border rounded-lg hover:bg-hover">
                {{ t("common.cancel") }}
              </button>
              <button
                type="submit"
    ...
    <div v-if="showTestDialog" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex min-h-screen items-center justify-center p-4">
        <div
          ...
          @click="showTestDialog = false"></div>
        <div
          class="relative modal-surface rounded-lg shadow-xl max-w-md w-full p-6">
          <h3 class="text-lg font-medium text-foreground mb-4">
          ...
          <div class="space-y-4">
            <div>
              <label
                class="block text-sm font-medium text-foreground-secondary"
                >{{ t("settings.smtp.testEmailTo") }}</label
              >
              <input
                v-model="testEmail"
                type="email"
...
const setDefault = async (config: SMTPConfig) => {
  try {
    ...
    appStore.showToast({
      type: "success",
      title: t("settings.smtp.setDefaultSuccess"),
    });
    fetchConfigs();
  } catch (error) {
...
const sendTestEmail = async () => {
  ...
  try {
    const response = await smtpApi.sendTestEmail(
      testingConfig.value.id,
      testEmail.value,
    );
    appStore.showToast({
      type: response.data.success ? "success" : "error",
