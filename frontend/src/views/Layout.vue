...
const expandedGroups = ref<string[]>(['overview', 'data-protection'])
...
<template>
 ...
 <!-- Category Label -->
 <div class="px-3 py-1.5 text-xs font-semibold text-foreground-muted uppercase tracking-wider">
 {{ category.name }}
 </div>
 <!-- Category Items (三级菜单) -->
 ...
 <!-- Divider -->
 <div v-if="group.id !== 'system'" class="my-3 border-t border-border"></div>
 </div>
 ...
 <!-- Popup Header -->
 <div class="px-3 py-2 text-xs font-semibold text-foreground-muted uppercase tracking-wider border-b border-border">
 {{ group.title }}
 </div>
 ...
 <div class="px-3 py-1.5 mt-2 text-xs font-semibold text-foreground-muted uppercase tracking-wider">
 {{ category.name }}
 </div>
 <router-link
 ...
 <div class="absolute bottom-0 left-0 right-0 p-3 border-t border-border surface-card">
 <button
 @click="toggleSidebar"
 ...
 locale === 'zh-CN' ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-foreground-secondary'
 ]"
 >
 中文
 </button>
 ...
 :class="[
 'w-full px-4 py-2 text-left text-sm hover:bg-hover',
 locale === 'en' ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-foreground-secondary'
 ]"
 >
 English
 </button>
 ...
 <div>
 <div class="font-medium text-foreground">{{ authStore.user?.full_name || authStore.user?.email }}</div>
 <div class="text-sm text-foreground-secondary">{{ t('welcome.subtitle') }}</div>
 </div>
 </div>
 </div>
 <div class="bg-background-secondary px-4 py-2 text-xs text-foreground-muted text-center">
