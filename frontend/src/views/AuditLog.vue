<template>
 ...
 <div class="text-xs text-foreground-secondary">{{ t('auditLog.totalLogs') }}</div>
 <div class="mt-1 text-xl font-semibold text-foreground">
 {{ statistics.total_count }}
 </div>
 </div>
 <div class="bg-card rounded-lg border border-border p-3">
 <div class="text-xs text-foreground-secondary">{{ t('auditLog.todayLogs') }}</div>
 <div class="mt-1 text-xl font-semibold text-foreground">
 {{ statistics.today_count }}
 </div>
 </div>
 <div class="bg-card rounded-lg border border-border p-3">
 <div class="text-xs text-foreground-secondary">{{ t('auditLog.successRate') }}</div>
 <div class="mt-1 text-xl font-semibold text-green-600 dark:text-green-400">
 {{ successRate }}%
 </div>
 </div>
 <div class="bg-card rounded-lg border border-border p-3">
 <div class="text-xs text-foreground-secondary">{{ t('auditLog.failureCount') }}</div>
 ...
 <div class="flex-1 min-w-[280px] flex rounded-lg overflow-hidden border border-border">
 ...
 searchType === type.value ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-foreground-secondary'
 ]"
 >
 {{ type.label }}
 </button>
 ...
 !filters.action ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-foreground-secondary'
 ]"
 >
 {{ t('common.all') }}
 </button>
 ...
 filters.action === action ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-foreground-secondary'
 ]"
 >
 ...
 :class="[
 'w-full px-3 py-2 text-left text-sm hover:bg-hover first:rounded-t-lg',
 !filters.resource_type ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-foreground-secondary'
 ]"
 >
 {{ t('common.all') }}
 </button>
 ...
 :class="[
 'w-full px-3 py-2 text-left text-sm hover:bg-hover last:rounded-b-lg',
 filters.resource_type === type ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-foreground-secondary'
 ]"
 >
 ...
 <div v-if="datePreset === 'custom'" class="mt-3 pt-3 border-t border-border flex items-center gap-3">
 <div class="flex items-center gap-2">
 <label class="text-sm text-foreground-secondary">{{ t('auditLog.startDate') }}</label>
 <input
 ...
 <div class="flex items-center gap-2">
 <label class="text-sm text-foreground-secondary">{{ t('auditLog.endDate') }}</label>
 <input
 ...
 <span class="mt-2 text-sm text-foreground-secondary">{{ t('common.loading') }}</span>
 </div>
 </td>
 </tr>
 ...
 <td :colspan="columns.length" class="px-4 py-12 text-center">
 <div class="flex flex-col items-center">
 <ClipboardDocumentListIcon class="w-12 h-12 text-foreground-muted" />
 <span class="mt-2 text-sm text-foreground-secondary">{{ t('common.noData') }}</span>
 </div>
 </td>
 </tr>
 <tr
 ...
 <div class="flex items-center gap-2">
 <span class="text-sm text-foreground-secondary">{{ t('common.rowsPerPage') }}</span>
 <select
 v-model="pageSize"
 ...
 <div class="relative modal-surface rounded-xl shadow-xl max-w-2xl w-full">
 <div class="flex items-center justify-between px-6 py-4 border-b border-border">
 <h3 class="text-lg font-semibold text-foreground">
 {{ t('auditLog.detail') }}
 </h3>
 <button @click="showDetailModal = false" class="p-1 rounded-lg text-slate-400 hover:bg-hover">
 ...
 <div>
 <label class="block text-xs font-medium text-foreground-secondary mb-1">
 ...
 <div class="text-sm text-foreground font-mono bg-background-secondary px-3 py-2 rounded">
 ...
 >
 <div v-show="showRequestDetails" class="px-4 py-3 space-y-3 bg-card">
 <!-- Query Parameters -->
 <div v-if="selectedLog.request_query && Object.keys(selectedLog.request_query).length > 0">
 <label class="block text-xs font-medium text-foreground-secondary mb-1">
 {{ t('auditLog.queryParams') }}
 </label>
 <pre class="bg-background-secondary rounded-lg p-3 text-xs overflow-auto max-h-40 font-mono text-foreground">{{ formatJson(selectedLog.request_query) }}</pre>
 </div>
 <!-- Request Body -->
 ...
 <pre class="bg-background-secondary rounded-lg p-3 text-xs overflow-auto max-h-60 font-mono text-foreground">{{ formatJson(selectedLog.request_body) }}</pre>
 </div>
 <!-- User Agent -->
 ...
 <div class="text-xs text-foreground-secondary bg-background-secondary px-3 py-2 rounded font-mono break-all">
 {{ selectedLog.user_agent }}
 </div>
 </div>
 <!-- Empty State -->
 <div v-if="(!selectedLog.request_query || Object.keys(selectedLog.request_query).length === 0) && (!selectedLog.request_body || Object.keys(selectedLog.request_body).length === 0)" class="text-sm text-foreground-secondary italic">
 ...
 <pre class="bg-background-secondary rounded-lg p-3 text-xs overflow-auto max-h-60 font-mono">{{ JSON.stringify(selectedLog.changes, null, 2) }}</pre>
 </div>
 </div>
 </div>
 </div>
