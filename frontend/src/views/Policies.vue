...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div class="bg-card rounded-xl border border-border p-4 shadow-sm">
        <p class="text-xs text-foreground-secondary">
          {{ t("policies.stats.disabled") }}
        </p>
        <p class="text-xl font-bold text-slate-400 mt-1">
          {{ policies.filter((p) => !p.enabled).length }}
        </p>
      </div>
    </div>
    ...
    <div v-if="isLoading" class="flex items-center justify-center py-12">
      <div
        class="w-8 h-8 border-4 border-violet-200 border-t-violet-600 rounded-full animate-spin"
      />
    </div>
              ...
              <div class="flex items-center gap-3">
                <div
                  :class="[
                    ...
                    getScheduleTypeColor(policy.schedule_type),
                  ]"
                >
                  <component
                    :is="getScheduleTypeIcon(policy.schedule_type)"
                    class="w-5 h-5"
                  />
                </div>
                <div>
      ...
      >
        ...
        >
          ...
          <div class="p-6 space-y-4">
            <div>
              ...
              <input
                ...
                type="text"
                class="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500"
              />
            </div>
            <div>
              <label
                class="block text-sm font-medium text-foreground-secondary mb-1"
                >{{ t("policies.form.backupTask") }}</label
              >
              <select
                v-model="newPolicy.backup_task"
                class="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500"
              >
                <option class="bg-background" :value="undefined">
                  Select Task
                </option>
                <option
                  class="bg-background"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label
                  class="block text-sm font-medium text-foreground-secondary mb-1"
                  >{{ t("policies.form.scheduleType") }}</label
                >
                <select
                  v-model="newPolicy.schedule_type"
                  class="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500"
                >
                  <option class="bg-background" value="hourly">
                    {{ t("policies.scheduleTypes.hourly") }}
                  </option>
                  <option class="bg-background" value="daily">
                    {{ t("policies.scheduleTypes.daily") }}
                  </option>
                  <option class="bg-background" value="weekly">
                    {{ t("policies.scheduleTypes.weekly") }}
                  </option>
                  <option class="bg-background" value="monthly">
                    {{ t("policies.scheduleTypes.monthly") }}
                  </option>
                </select>
              </div>
              <div>
                <label
                  class="block text-sm font-medium text-foreground-secondary mb-1"
                  >{{ t("policies.form.retention") }}</label
                >
                <input
                  v-model.number="newPolicy.retention_days"
                  type="number"
                  min="1"
                  class="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500"
                />
            ...
            <div v-if="newPolicy.schedule_type === 'daily'">
              <label
                class="block text-sm font-medium text-foreground-secondary mb-1"
                >{{ t("policies.form.time") }}</label
              >
              <input
                v-model="newPolicy.schedule_config!.time"
                type="time"
                class="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500"
              />
            </div>
          ...
          <div class="px-6 py-4 border-t border-border flex justify-end gap-3">
            ...
            <button
              @click="createPolicy"
              class="px-4 py-2 text-sm text-white bg-violet-600 rounded-lg hover:bg-violet-700"
            >
              {{ t("common.create") }}
            </button>
          </div>
