...
<template>
  <div class="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-black dark:via-neutral-950 dark:to-black p-4">
 <!-- Animated Background -->
 ...
 <h1 class="text-xl font-bold text-foreground tracking-tight">{{ t('auth.forgotPassword') }}</h1>
 <p class="text-sm text-foreground-secondary mt-1">{{ t('auth.forgotPasswordSubtitle') }}</p>
 </div>

 <!-- Progress Steps -->
 <div class="flex items-center justify-center gap-1 mb-5">
 <div :class="['w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium transition-colors', currentStep >= 1 ? 'bg-purple-500 text-white' : 'bg-slate-200 text-slate-400']">1</div>
 <div :class="['w-6 h-0.5 transition-colors', currentStep >= 2 ? 'bg-purple-500' : 'bg-slate-200']"></div>
 <div :class="['w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium transition-colors', currentStep >= 2 ? 'bg-purple-500 text-white' : 'bg-slate-200 text-slate-400']">2</div>
 <div :class="['w-6 h-0.5 transition-colors', currentStep >= 3 ? 'bg-purple-500' : 'bg-slate-200']"></div>
 <div :class="['w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium transition-colors', currentStep >= 3 ? 'bg-purple-500 text-white' : 'bg-slate-200 text-slate-400']">3</div>
 <div :class="['w-6 h-0.5 transition-colors', currentStep >= 4 ? 'bg-purple-500' : 'bg-slate-200']"></div>
 <div :class="['w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium transition-colors', currentStep >= 4 ? 'bg-purple-500 text-white' : 'bg-slate-200 text-slate-400']">
 ...
 <!-- Form Card -->
 <div class="bg-card/90 backdrop-blur-xl rounded-2xl shadow-xl dark:shadow-2xl border border-border p-5">
 <!-- Error Message -->
 ...
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.email') }}</label>
 <div class="relative">
 ...
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.captcha') }}</label>
 ...
 <div class="relative h-9 w-24 rounded-lg border border-border bg-background-secondary cursor-pointer overflow-hidden hover:border-purple-500/50 transition-colors" @click="refreshCaptcha" :title="t('auth.refreshCaptcha')">
 <img
 v-if="captchaUrl"
 :src="captchaUrl"
 alt="Captcha"
 ...
 <p class="text-sm text-foreground-secondary">{{ t('auth.enterVerificationCode') }}</p>
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.verificationCode') }}</label>
 <input
 ...
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.newPassword') }}</label>
 <div class="relative">
 <div class="absolute left-3 top-1/2 -translate-y-1/2 text-foreground-muted">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <div>
 <label class="block text-sm text-foreground-secondary mb-1">{{ t('auth.confirmPassword') }}</label>
 <div class="relative">
 <div class="absolute left-3 top-1/2 -translate-y-1/2 text-foreground-muted">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
 ...
 :class="confirmPassword && newPassword !== confirmPassword ? 'border-red-400 dark:border-red-500/50' : 'border-border'"
 ...
 <h3 class="text-lg font-medium text-foreground mb-1">{{ t('auth.passwordResetSuccess') }}</h3>
 <p class="text-sm text-foreground-secondary mb-4">{{ t('auth.canNowLogin') }}</p>
 <router-link
 to="/login"
 ...
 <!-- Back to Login -->
 <div v-if="currentStep < 4" class="mt-4 pt-4 border-t border-border text-center">
 <router-link to="/login" class="text-sm text-foreground-secondary hover:text-slate-700 dark:hover:text-white inline-flex items-center gap-1 transition-colors">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
