...
const selectedTestResult = ref<{ success: boolean; message: string; details?: any } | null>(null)
...
const connectionTestResult = ref<Record<string, { success: boolean; message: string; details?: any }>>({})
...
async function createRepository() {
 ...
 quota_bytes: newRepo.value.quota ? newRepo.value.quota * 1024 * 1024 * 1024 : 0, // Convert GB to bytes
...
async function testConnection(repo: Repository) {
 ...
 if (details && (details.connectivity || details.write_test || details.space_info)) {
...
function openEditModal(repo: Repository) {
 ...
 // 回填配额信息，将字节转换为 GB
 newRepo.value.quota = repo.quota_bytes ? Math.round(repo.quota_bytes / (1024 * 1024 * 1024)) : 0
...
<template>
 ...
 <div>
 <h1 class="text-2xl font-bold text-foreground">{{ t('repository.title') }}</h1>
 <p class="text-foreground-secondary mt-1">{{ t('repository.subtitle') }}</p>
 </div>
 <button
 ...
 <p class="text-xs text-foreground-secondary">{{ t('repository.stats.total') }}</p>
 <p class="text-xl font-bold text-foreground mt-1">{{ stats.total }}</p>
 </div>
 <div class="bg-card rounded-xl border border-border p-4 shadow-sm">
 <p class="text-xs text-foreground-secondary">{{ t('repository.stats.active') }}</p>
 <p class="text-xl font-bold text-emerald-600 mt-1">{{ stats.active }}</p>
 </div>
 <div class="bg-card rounded-xl border border-border p-4 shadow-sm">
 <p class="text-xs text-foreground-secondary">{{ t('repository.stats.totalCapacity') }}</p>
 <p class="text-xl font-bold text-foreground mt-1">{{ formatBytes(stats.totalCapacity) }}</p>
 </div>
 <div class="bg-card rounded-xl border border-border p-4 shadow-sm">
 <p class="text-xs text-foreground-secondary">{{ t('repository.stats.usedSpace') }}</p>
 ...
 <MagnifyingGlassIcon class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground-muted" />
 <input
 v-model="searchQuery"
 type="text"
 ...
 >
 <option class="bg-background/50" value="">{{ t('sourceResources.allTypes') }}</option>
 <option class="bg-background/50" value="s3">S3</option>
 <option class="bg-background/50" value="local">{{ t('repository.types.local') }}</option>
 ...
 viewMode === 'card' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-foreground-muted hover:text-foreground-secondary dark:hover:text-slate-300 hover:bg-hover/50'
 ]"
 ...
 :class="[
 'p-2 rounded-md transition-colors',
 viewMode === 'list' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-foreground-muted hover:text-foreground-secondary dark:hover:text-slate-300 hover:bg-hover/50'
 ]"
 ...
 <div v-else-if="filteredRepos.length === 0" class="bg-background-secondary rounded-xl border border-border p-12 text-center">
 <div class="w-16 h-16 bg-background-tertiary/50 rounded-full flex items-center justify-center mx-auto mb-4">
 <CircleStackIcon class="w-8 h-8 text-foreground-muted" />
 </div>
 <h3 class="text-lg font-medium text-foreground mb-1">{{ t('repository.empty.title') }}</h3>
 <p class="text-foreground-secondary">{{ t('repository.empty.description') }}</p>
 </div>
 ...
 repo.status === 'active' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 dark:text-emerald-400' : 'bg-background-tertiary/50 text-foreground-secondary'
 ]">
 ...
 <p class="text-sm text-foreground-secondary mb-3">{{ repo.description || getRepoTypeLabel(repo.repo_type) }}</p>
 ...
 <span class="text-foreground-secondary truncate" :title="repo.config?.endpoint">{{ repo.config?.endpoint || '-' }}</span>
 </div>
 <div class="flex items-center gap-2 text-sm">
 <FolderIcon class="w-4 h-4 text-foreground-muted flex-shrink-0" />
 <span class="text-foreground-secondary">{{ repo.config?.bucket || '-' }}</span>
 <span v-if="repo.config?.prefix" class="text-foreground-muted">/{{ repo.config?.prefix }}</span>
 </div>
 </template>
 ...
 <span class="text-foreground-secondary">{{ repo.config?.server || '-' }}</span>
 </div>
 <div class="flex items-center gap-2 text-sm">
 <FolderIcon class="w-4 h-4 text-foreground-muted flex-shrink-0" />
 <span class="text-foreground-secondary">{{ repo.config?.export_path || '-' }}</span>
 </div>
 <div class="flex items-center gap-2 text-sm">
 <span class="text-xs text-foreground-muted uppercase px-1.5 py-0.5 bg-slate-200 dark:bg-slate-600 rounded">{{ repo.config?.nas_type?.toUpperCase() || 'NFS' }}</span>
 </div>
 </template>
 ...
 <div class="flex items-center gap-2 text-sm">
 <FolderIcon class="w-4 h-4 text-foreground-muted flex-shrink-0" />
 <span class="text-foreground-secondary truncate" :title="repo.config?.path">{{ repo.config?.path || '-' }}</span>
 </div>
 </template>
 
 <!-- Bound Node -->
 <div class="flex items-center gap-2 text-sm pt-2 border-t border-border">
 <LinkIcon class="w-4 h-4 text-foreground-muted flex-shrink-0" />
 <span class="text-foreground-secondary">{{ getNodeName(repo.bound_node) }}</span>
 </div>
 </div>
 
 <!-- Storage Progress -->
 <div class="mb-4">
 <div class="flex items-center justify-between text-xs text-foreground-secondary mb-1">
 <span>{{ t('repository.stats.usedSpace') }}</span>
 <span>{{ formatBytes(repo.used_space || 0) }} / {{ formatBytes(repo.capacity || 0) }}</span>
 </div>
 <div class="h-2 bg-background-tertiary/50 rounded-full overflow-hidden relative">
 ...
 :style="{ width: `${Math.min(((repo.quota_bytes || 0) / (repo.capacity || 1)) * 100, 100)}%` }"
 />
 <!-- Actual usage bar -->
 <div
 ...
 <!-- Quota Information -->
 <div v-if="repo.quota_enabled && (repo.quota_bytes || 0) > 0" class="mt-2 flex items-center justify-between text-xs">
 <div class="flex items-center gap-1">
 <ExclamationCircleIcon
 v-if="repo.quota_status === 'warning' || repo.quota_status === 'critical'"
 :class="[
 'w-3.5 h-3.5',
 repo.quota_status === 'critical' ? 'text-red-500 dark:text-red-400' : 'text-amber-500 dark:text-amber-400'
 ]"
 />
 <span class="text-foreground-secondary">
 {{ t('repository.stats.quota') }}: {{ repo.quota_bytes_formatted }}
 </span>
 </div>
 <span :class="[
 'text-xs font-medium',
 repo.quota_status === 'critical' ? 'text-red-500 dark:text-red-400' :
 repo.quota_status === 'warning' ? 'text-amber-500 dark:text-amber-400' :
 'text-foreground-secondary'
 ]">
 {{ repo.quota_usage_percentage?.toFixed(1) || 0 }}%
 </span>
 </div>
 <div v-else-if="repo.quota_enabled" class="mt-2 flex items-center justify-between text-xs text-foreground-muted">
 <span>{{ t('repository.stats.quota') }}: {{ t('repository.stats.unlimited') }}</span>
 </div>
 </div>
 
 <div class="flex items-center justify-between pt-3 border-t border-border">
 <span class="text-xs text-foreground-muted uppercase">{{ getRepoTypeLabel(repo.repo_type) }}</span>
 <div class="flex items-center gap-1">
 <button
 ...
 <div v-else-if="filteredRepos.length === 0" class="rounded-xl border border-border p-12 text-center">
 <div class="w-16 h-16 bg-background-tertiary/50 rounded-full flex items-center justify-center mx-auto mb-4">
 <CircleStackIcon class="w-8 h-8 text-foreground-muted" />
 </div>
 <h3 class="text-lg font-medium text-foreground mb-1">{{ t('repository.empty.title') }}</h3>
 ...
 <tr>
 <th class="sticky left-0 bg-background-secondary px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider z-10">{{ t('repository.list.name') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.type') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.status') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.connection') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.boundNode') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.capacity') }}</th>
 <th class="px-4 py-3 text-left text-xs font-medium text-foreground-secondary uppercase tracking-wider whitespace-nowrap">{{ t('repository.list.kopia') }}</th>
 <th class="sticky right-0 bg-background-secondary px-4 py-3 text-right text-xs font-medium text-foreground-secondary uppercase tracking-wider z-10">{{ t('repository.list.actions') }}</th>
 </tr>
 </thead>
 ...
 >
 <!-- Name -->
 <td class="sticky left-0 bg-card px-4 py-3 whitespace-nowrap z-10">
 ...
 repo.repo_type === 's3' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400' :
 repo.repo_type === 'nas' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 dark:text-purple-400' :
 ...
 <!-- Connection Info -->
 <td class="px-4 py-3 text-sm text-foreground-secondary max-w-[200px]">
 <template v-if="repo.repo_type === 's3'">
 ...
 <!-- Bound Node -->
 <td class="px-4 py-3 whitespace-nowrap text-sm text-foreground-secondary">
 ...
 <div class="flex-1 h-1.5 bg-slate-200 dark:bg-slate-600 dark:bg-slate-600 rounded-full overflow-hidden relative">
 <!-- Quota indicator (subtle overlay) -->
 <div
 v-if="repo.quota_enabled && (repo.quota_bytes || 0) > 0"
 class="h-full absolute left-0 bg-slate-300 dark:bg-slate-600/50 dark:bg-slate-600/50 opacity-50 rounded-full transition-all"
 :style="{ width: `${Math.min(((repo.quota_bytes || 0) / repo.capacity) * 100, 100)}%` }"
 />
 <!-- Usage bar -->
 <div
 class="h-full rounded-full transition-all relative"
 :class="getProgressColor(repo.quota_status || 'unlimited')"
 :style="{ width: `${((repo.used_space || 0) / repo.capacity) * 100}%` }"
 />
 </div>
 </div>
 <!-- Text info -->
 <div class="text-xs text-foreground-secondary">
 <div>{{ formatBytes(repo.used_space || 0) }} / {{ formatBytes(repo.capacity) }}</div>
 <div v-if="repo.quota_enabled && (repo.quota_bytes || 0) > 0" class="flex items-center gap-1 mt-0.5">
 <span class="text-foreground-muted">{{ t('repository.stats.quota') }}:</span>
 <span
 :class="[
 'font-medium',
 repo.quota_status === 'critical' ? 'text-red-500 dark:text-red-400 dark:text-red-400' :
 repo.quota_status === 'warning' ? 'text-amber-500 dark:text-amber-400 dark:text-amber-400' :
 'text-foreground-secondary'
 ]"
 >
 ...
 <!-- Kopia -->
 <td class="px-4 py-3 whitespace-nowrap">
 <span v-if="repo.kopia_initialized" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 dark:text-blue-400">
 <CheckCircleIcon class="w-3 h-3" />
 ...
 <!-- Actions -->
 <td class="sticky right-0 bg-card px-4 py-3 whitespace-nowrap text-right z-10">
 ...
 <div class="relative modal-surface rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
 <!-- Fixed Header -->
 ...
 <button @click="showCreateModal = false; resetForm()" class="p-1 hover:bg-background-tertiary/50 rounded-lg">
 ...
 <label class="block text-sm font-medium text-foreground mb-3">{{ t('repository.form.repositoryType') }}</label>
 ...
 <span class="text-xs font-medium text-foreground dark:text-slate-200">{{ type.label }}</span>
 </button>
 </div>
 </div>
 ...
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('common.name') }} *</label>
 <input 
 v-model="newRepo.name" 
 type="text" 
 :placeholder="t('repository.form.namePlaceholder')" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2', 
 formErrors.name ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('name')"
 />
 <p v-if="formErrors.name" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.name }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('common.description') }}</label>
 <input v-model="newRepo.description" type="text" :placeholder="t('repository.form.descPlaceholder')" class="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500" />
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">
 {{ t('repository.form.storageQuota') }}
 <span class="text-xs text-foreground-muted font-normal ml-1">({{ t('repository.form.quotaUnit') }})</span>
 </label>
 <input
 ...
 />
 <p class="mt-1 text-xs text-foreground-muted">{{ t('repository.form.quotaHint') }}</p>
 </div>
 ...
 <div v-if="newRepo.quota_enabled" class="flex items-center gap-2 mt-2">
 <span class="text-sm text-foreground-secondary">{{ t('repository.form.quotaThreshold') }}:</span>
 <input
 ...
 <p class="text-xs text-foreground-muted">{{ t('repository.form.quotaThresholdHint') }}</p>
 </div>
 </div>
 </div>

 <!-- S3 Configuration -->
 <div v-if="newRepo.repo_type === 's3'" class="space-y-4 p-4 rounded-xl border border-border">
 <div class="flex items-start gap-2 text-sm text-orange-700 dark:text-orange-400 bg-orange-50 rounded-lg p-3 border-l-4 border-orange-400">
 ...
 <h4 class="text-sm font-medium text-foreground mb-3">{{ t('repository.s3.credentials') }}</h4>
 <div class="grid grid-cols-2 gap-4">
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.endpoint') }} *</label>
 <input 
 v-model="newRepo.s3_config.endpoint" 
 type="text" 
 placeholder="https://s3.amazonaws.com" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.endpoint ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('endpoint')"
 />
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.s3.endpointHint') }}</p>
 <p v-if="formErrors.endpoint" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.endpoint }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.region') }}</label>
 <input v-model="newRepo.s3_config.region" type="text" placeholder="us-east-1" class="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500" />
 </div>
 
 <!-- URL Style Selection -->
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.urlStyle') }}</label>
 <select 
 v-model="newRepo.s3_config.url_style"
 class="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500"
 >
 <option class="bg-background/50" value="virtual">{{ t('repository.s3.urlStyleVirtual') }}</option>
 <option class="bg-background/50" value="path">{{ t('repository.s3.urlStylePath') }}</option>
 </select>
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.s3.urlStyleHint') }}</p>
 </div>
 
 <!-- Use TLS Toggle -->
 <div class="flex items-center justify-between py-2">
 <div>
 <label class="block text-sm font-medium text-foreground dark:text-slate-200">{{ t('repository.s3.useTLS') }}</label>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.useTLSHint') }}</p>
 </div>
 <button 
 type="button"
 ...
 newRepo.s3_config.use_tls ? 'bg-blue-600' : 'bg-slate-200 dark:bg-slate-600'
 ]"
 role="switch"
 ...
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.accessKey') }} *</label>
 <input 
 v-model="newRepo.s3_config.access_key" 
 type="text" 
 placeholder="AKIAIOSFODNN7EXAMPLE" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.access_key ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('access_key')"
 />
 <p v-if="formErrors.access_key" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.access_key }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">
 ...
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.secret_key ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('secret_key')"
 />
 <p v-if="isEditMode && !formErrors.secret_key" class="mt-1 text-xs text-foreground-secondary">
 {{ t('repository.s3.secretKeyEditHint') }}
 </p>
 <p v-if="formErrors.secret_key" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.secret_key }}</p>
 </div>
 </div>
 </div>
 ...
 <div class="p-3 bg-background/50 rounded-lg border border-border">
 <h4 class="text-sm font-medium text-foreground mb-3">{{ t('repository.s3.bucketSelection') }}</h4>
 ...
 <div class="flex items-start gap-2 text-sm text-amber-700 bg-amber-50 rounded-lg p-3 mb-4 border border-amber-300 dark:border-amber-700 border-l-4">
 ...
 <!-- Bucket list error -->
 <p v-if="bucketListError" class="mb-3 text-xs text-red-500 dark:text-red-400">{{ bucketListError }}</p>
 
 <!-- Bucket select -->
 <div v-if="s3BucketList.length > 0">
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.selectBucket') }} *</label>
 <select
 v-model="newRepo.s3_config.bucket"
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.bucket ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @change="clearError('bucket')"
 >
 <option class="bg-background/50" value="">{{ t('repository.s3.selectBucketPlaceholder') }}</option>
 <option class="bg-background/50" v-for="bucket in s3BucketList" :key="bucket.name" :value="bucket.name">
 {{ bucket.name }}
 </option>
 </select>
 <p v-if="formErrors.bucket" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.bucket }}</p>
 </div>
 </div>
 ...
 <div class="flex items-start gap-2 text-sm text-foreground-secondary rounded-lg p-3 mb-4 border border-border">
 <InformationCircleIcon class="w-5 h-5 flex-shrink-0 mt-0.5 text-foreground-muted" />
 <div class="text-xs space-y-1">
 <p class="font-medium text-foreground dark:text-slate-200">{{ t('repository.s3.bucketNameRules') }}:</p>
 ...
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.bucketName') }} *</label>
 <div class="relative">
 <input 
 v-model="newRepo.s3_config.bucket" 
 type="text" 
 placeholder="my-backup-bucket" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 pr-24',
 ...
 bucketNameAvailable === true ? 'text-green-600' : 'text-red-500 dark:text-red-400'
 ]">
 ...
 <!-- Prefix -->
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.s3.prefix') }}</label>
 <input v-model="newRepo.s3_config.prefix" type="text" placeholder="backups/hyperfilelens" class="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500" />
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.s3.prefixHint') }}</p>
 </div>
 </div>

 <!-- NAS Configuration -->
 <div v-if="newRepo.repo_type === 'nas'" class="space-y-4 p-4 rounded-xl border border-border">
 <div class="flex items-start gap-2 text-sm text-purple-700 dark:text-purple-400 bg-purple-50 rounded-lg p-3 border-l-4 border-purple-400">
 <ExclamationCircleIcon class="w-5 h-5 flex-shrink-0 mt-0.5" />
 <div>
 ...
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.nas.mountType') }}</label>
 <div class="grid grid-cols-2 gap-3">
 <button
 ...
 <div class="grid grid-cols-2 gap-4">
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.nas.server') }} *</label>
 <input 
 v-model="newRepo.nas_config.server" 
 type="text" 
 placeholder="192.168.1.100 或 nas.example.com" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.server ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('server')"
 />
 <p v-if="formErrors.server" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.server }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.nas.exportPath') }} *</label>
 <input 
 v-model="newRepo.nas_config.export_path" 
 type="text" 
 :placeholder="newRepo.nas_config.mount_type === 'nfs' ? '/export/backup' : '/share/backup'" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.export_path ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('export_path')"
 />
 <p v-if="formErrors.export_path" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.export_path }}</p>
 </div>
 </div>

 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.nas.mountOptions') }}</label>
 <input v-model="newRepo.nas_config.mount_options" type="text" :placeholder="newRepo.nas_config.mount_type === 'nfs' ? 'rw,hard,intr' : 'vers=3.0,iocharset=utf8'" class="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2 focus:ring-blue-500" />
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.nas.mountOptionsHint') }}</p>
 </div>

 <!-- CIFS credentials -->
 <div v-if="newRepo.nas_config.mount_type === 'cifs'" class="grid grid-cols-2 gap-4 p-3 bg-background/50 rounded-lg">
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.nas.username') }} *</label>
 <input 
 v-model="newRepo.nas_config.username" 
 type="text" 
 placeholder="username" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.username ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('username')"
 />
 <p v-if="formErrors.username" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.username }}</p>
 </div>
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">
 ...
 placeholder="••••••••" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.password ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @input="clearError('password')"
 />
 <p v-if="isEditMode && !formErrors.password" class="mt-1 text-xs text-foreground-secondary">
 {{ t('repository.s3.secretKeyEditHint') }}
 </p>
 <p v-if="formErrors.password" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.password }}</p>
 </div>
 </div>

 <!-- Bound Sync Proxy for NAS -->
 <div>
 <label class="block text-sm font-medium text-foreground mb-1">{{ t('repository.boundSyncProxy') }} *</label>
 <select 
 v-model="newRepo.bound_node" 
 :class="['w-full px-3 py-2 text-sm border rounded-lg bg-background/50 text-foreground focus:outline-none focus:ring-2',
 formErrors.bound_node ? 'border-red-300 focus:ring-red-500' : 'border-border focus:ring-blue-500']"
 @change="clearError('bound_node')"
 >
 <option class="bg-background/50" value="">{{ t('repository.selectSyncProxy') }}</option>
 <option class="bg-background/50" v-for="proxy in availableSyncProxies" :key="proxy.id" :value="proxy.id">
 {{ proxy.name }} ({{ proxy.hostname || proxy.id }}) - {{ proxy.is_online ? t('proxies.status.online') : t('proxies.status.offline') }}
 </option>
 </select>
 <p v-if="formErrors.bound_node" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.bound_node }}</p>
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.boundSyncProxyHint') }}</p>
 ...
 <div v-if="newRepo.repo_type === 'local'" class="space-y-4 p-4 rounded-xl border border-border">
 <div class="flex items-start gap-2 text-sm text-blue-700 dark:text-blue-400 bg-blue-50 rounded-lg p-3 border-l-4 border-blue-400">
 <ExclamationCircleIcon class="w-5 h-5 flex-shrink-0 mt-0.5" />
 <div>
 ...
 <option class="bg-background/50" value="">{{ t('repository.selectSyncProxy') }}</option>
 <option class="bg-background/50" v-for="proxy in availableSyncProxies" :key="proxy.id" :value="proxy.id">
 {{ proxy.name }} ({{ proxy.hostname || proxy.id }}) - {{ t('proxies.status.online') }}
 </option>
 </select>
 <p v-if="formErrors.bound_node" class="mt-1 text-xs text-red-500 dark:text-red-400">{{ formErrors.bound_node }}</p>
 <p class="text-xs text-foreground-secondary mt-1">{{ t('repository.boundSyncProxyHint') }}</p>
 ...
 <!-- Directory Browser -->
 <div v-if="newRepo.bound_node" class="bg-background/50 rounded-lg border border-border">
 <div class="px-3 py-2 border-b border-border flex items-center justify-between">
 <div class="flex items-center gap-2 text-sm">
 <FolderIcon class="w-4 h-4 text-foreground-muted" />
 <span class="text-foreground-secondary">{{ t('repository.local.selectDirectory') }}</span>
 </div>
 <button
 v-if="currentPath !== '/'"
 @click="navigateUp"
 ...
 <code class="text-sm text-foreground dark:text-slate-200">{{ currentPath || '/' }}</code>
 </div>
 ...
 <div v-if="proxyDirectories.length === 0" class="py-6 text-center text-sm text-foreground-secondary">
 ...
 <!-- Path Error -->
 <p v-if="formErrors.path" class="text-xs text-red-500 dark:text-red-400">{{ formErrors.path }}</p>
 ...
 <!-- Fixed Footer -->
 <div class="px-6 py-4 border-t border-border flex justify-end gap-3 flex-shrink-0 bg-card">
 <button @click="showCreateModal = false; resetForm()" class="px-4 py-2 text-sm text-foreground-secondary border border-border rounded-lg hover:bg-hover/50 transition-colors">
 {{ t('common.cancel') }}
 </button>
 <button 
 ...
 <div class="relative modal-surface rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
 <div class="px-6 py-4 border-b border-border flex items-center justify-between sticky top-0 modal-surface z-10">
 <h2 class="text-lg font-semibold text-foreground">{{ selectedRepo.name }}</h2>
 <button @click="showDetailModal = false" class="p-1 hover:bg-background-tertiary/50 rounded-lg">
 <svg class="w-5 h-5 text-foreground-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
 </svg>
 </button>
 ...
 <div class="flex-1">
 <p class="font-medium text-foreground dark:text-slate-200">{{ getRepoTypeLabel(selectedRepo.repo_type) }}</p>
 <p class="text-sm text-foreground-secondary">{{ selectedRepo.status === 'active' ? t('common.active') : t('common.inactive') }}</p>
 </div>
 ...
 <!-- Description -->
 <div v-if="selectedRepo.description" class="rounded-lg p-4">
 <p class="text-xs text-foreground-secondary mb-1">{{ t('common.description') }}</p>
 <p class="text-sm text-foreground dark:text-slate-200">{{ selectedRepo.description }}</p>
 </div>
 ...
 <!-- Quota Information -->
 <div v-if="selectedRepo.quota_enabled || (selectedRepo.quota_bytes && selectedRepo.quota_bytes > 0)" class="bg-background-secondary rounded-lg p-4 space-y-3">
 <div class="flex items-center justify-between">
 <h4 class="font-medium text-foreground dark:text-slate-200 flex items-center gap-2">
 ...
 <span v-if="!selectedRepo.quota_enabled" class="text-xs px-2 py-1 rounded-full bg-slate-200 dark:bg-slate-600 dark:bg-slate-600 text-foreground-secondary">
 {{ t('repository.stats.disabled') }}
 </span>
 <span v-else class="text-xs px-2 py-1 rounded-full"
 :class="{
 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 dark:text-emerald-400': selectedRepo.quota_status === 'ok',
 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400': selectedRepo.quota_status === 'warning',
 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400': selectedRepo.quota_status === 'critical'
 }">
 {{ selectedRepo.quota_status === 'ok' ? t('common.statusValues.normal') :
 selectedRepo.quota_status === 'warning' ? t('common.statusValues.warning') :
 selectedRepo.quota_status === 'critical' ? t('common.statusValues.critical') : '' }}
 </span>
 </div>

 <!-- Quota Progress Bar -->
 <div v-if="selectedRepo.quota_enabled && (selectedRepo.quota_bytes || 0) > 0" class="space-y-2">
 <div class="flex items-center justify-between text-xs">
 <span class="text-foreground-secondary">{{ t('repository.stats.usage') }}</span>
 <span class="font-medium"
 :class="{
 'text-emerald-600 dark:text-emerald-400': selectedRepo.quota_status === 'ok',
 'text-amber-600 dark:text-amber-400': selectedRepo.quota_status === 'warning',
 'text-red-600 dark:text-red-400': selectedRepo.quota_status === 'critical'
 }">
 {{ selectedRepo.quota_usage_percentage?.toFixed(1) || 0 }}%
 </span>
 </div>
 <!-- Combined Progress Bar -->
 <div class="h-3 bg-slate-200 dark:bg-slate-600 dark:bg-slate-600 rounded-full overflow-hidden relative">
 <!-- Quota Limit (subtle overlay) -->
 <div class="absolute left-0 top-0 h-full bg-purple-200 dark:bg-purple-900/30 opacity-50"
 :style="{ width: `${Math.min(((selectedRepo.quota_bytes || 0) / (selectedRepo.capacity || selectedRepo.quota_bytes || 1)) * 100, 100)}%` }">
 </div>
 <!-- Actual Usage -->
 <div class="h-full rounded-full relative"
 :class="getProgressColor(selectedRepo.quota_status || 'unlimited')"
 :style="{ width: `${selectedRepo.capacity ? ((selectedRepo.used_space || 0) / selectedRepo.capacity) * 100 : 0}%` }">
 </div>
 </div>
 <!-- Legend -->
 <div class="flex items-center gap-4 text-xs">
 <div class="flex items-center gap-1">
 <div class="w-3 h-3 rounded bg-purple-200 dark:bg-purple-900/30"></div>
 <span class="text-foreground-secondary">{{ t('repository.stats.quotaLimit') }}</span>
 </div>
 <div class="flex items-center gap-1">
 <div :class="['w-3 h-3 rounded', getProgressColor(selectedRepo.quota_status || 'unlimited')]"></div>
 <span class="text-foreground-secondary">{{ t('repository.stats.usage') }}</span>
 </div>
 </div>
 </div>
 ...
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.form.storageQuota') }}</p>
 <p class="font-medium text-foreground dark:text-slate-200">{{ selectedRepo.quota_bytes_formatted || t('repository.stats.unlimited') }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.form.quotaThreshold') }}</p>
 <p class="font-medium text-foreground dark:text-slate-200">{{ selectedRepo.quota_warning_threshold || 80 }}%</p>
 </div>
 </div>
 </div>

 <!-- S3 Configuration -->
 <div v-if="selectedRepo.repo_type === 's3'" class="bg-background-secondary rounded-lg p-4 space-y-3">
 <h4 class="font-medium text-foreground dark:text-slate-200 flex items-center gap-2">
 ...
 <div class="grid grid-cols-2 gap-3 text-sm">
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.endpoint') }}</p>
 <p class="text-foreground dark:text-slate-200 font-mono">{{ selectedRepo.config?.endpoint || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.bucket') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.bucket || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.region') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.region || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.prefix') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.prefix || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.accessKey') }}</p>
 <p class="text-foreground dark:text-slate-200 font-mono">{{ selectedRepo.credentials_masked?.access_key || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.s3.useTLS') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.use_tls !== false ? t('common.yes') : t('common.no') }}</p>
 </div>
 </div>
 </div>

 <!-- NAS Configuration -->
 <div v-if="selectedRepo.repo_type === 'nas'" class="bg-background-secondary rounded-lg p-4 space-y-3">
 <h4 class="font-medium text-foreground dark:text-slate-200 flex items-center gap-2">
 ...
 <div class="grid grid-cols-2 gap-3 text-sm">
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.nas.server') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.server || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.nas.exportPath') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.export_path || '-' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.nas.nasType') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.nas_type?.toUpperCase() || 'NFS' }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.nas.mountOptions') }}</p>
 <p class="text-foreground dark:text-slate-200 font-mono text-xs">{{ selectedRepo.config?.mount_options || '-' }}</p>
 </div>
 <div v-if="selectedRepo.config?.nas_type === 'cifs'">
 <p class="text-xs text-foreground-secondary">{{ t('repository.nas.username') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.config?.username || '-' }}</p>
 </div>
 </div>
 </div>

 <!-- Local Configuration -->
 <div v-if="selectedRepo.repo_type === 'local'" class="bg-background-secondary rounded-lg p-4 space-y-3">
 <h4 class="font-medium text-foreground dark:text-slate-200 flex items-center gap-2">
 <FolderIcon class="w-5 h-5" />
 {{ t('repository.types.local') }} {{ t('repository.configInfo') }}
 </h4>
 <div class="text-sm">
 <p class="text-xs text-foreground-secondary">{{ t('repository.local.path') }}</p>
 <p class="text-foreground dark:text-slate-200 font-mono">{{ selectedRepo.config?.path || '-' }}</p>
 </div>
 </div>

 <!-- Bound Node -->
 <div class="bg-background-secondary rounded-lg p-4">
 <p class="text-xs text-foreground-secondary mb-1">{{ t('sourceResources.boundNode') }}</p>
 <div class="flex items-center gap-2">
 <div :class="['w-2 h-2 rounded-full', getNodeStatus(selectedRepo.bound_node) === 'online' ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600/50']" />
 <span class="text-sm text-foreground dark:text-slate-200">{{ getNodeName(selectedRepo.bound_node) }}</span>
 <span v-if="getNode(selectedRepo.bound_node)?.role" class="text-xs text-foreground-muted">({{ getNode(selectedRepo.bound_node)?.role }})</span>
 </div>
 </div>
 ...
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('common.createdAt') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ new Date(selectedRepo.created_at).toLocaleString() }}</p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('common.updatedAt') }}</p>
 <p class="text-foreground dark:text-slate-200">{{ selectedRepo.updated_at ? new Date(selectedRepo.updated_at).toLocaleString() : '-' }}</p>
 </div>
 </div>
 </div>
 <div class="px-6 py-4 border-t border-border flex justify-end sticky bottom-0 modal-surface">
 <button @click="showDetailModal = false" class="px-4 py-2 text-sm text-foreground-secondary border border-border rounded-lg hover:bg-hover/50">
 {{ t('common.cancel') }}
 </button>
 </div>
 </div>
 ...
 <!-- Test Result Modal -->
 <Teleport to="body">
 <div v-if="showTestResultModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
 <div class="absolute inset-0 bg-black/50" @click="showTestResultModal = false" />
 <div class="relative modal-surface rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
 <!-- Header -->
 <div class="flex items-center justify-between px-6 py-4 border-b border-border">
 <div class="flex items-center gap-3">
 <div :class="['w-10 h-10 rounded-full flex items-center justify-center', selectedTestResult?.success ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-red-100 dark:bg-red-900/30']">
 <CheckCircleIcon v-if="selectedTestResult?.success" class="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
 <XCircleIcon v-else class="w-6 h-6 text-red-600 dark:text-red-400" />
 </div>
 <div>
 <h3 class="text-lg font-semibold text-foreground">{{ t('repository.connectionTestResult') }}</h3>
 <p class="text-sm text-foreground-secondary">{{ selectedTestResult?.message }}</p>
 </div>
 </div>
 <button @click="showTestResultModal = false" class="p-2 hover:bg-background-tertiary/50 rounded-lg transition-colors">
 <XMarkIcon class="w-5 h-5 text-foreground-muted" />
 </button>
 </div>

 <!-- Content -->
 <div v-if="selectedTestResult?.details" class="p-6">
 <div class="space-y-4">
 <!-- Connectivity -->
 <div v-if="selectedTestResult.details.connectivity" class="bg-background-secondary rounded-lg p-4">
 <div class="flex items-center gap-2 mb-2">
 <SignalIcon class="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
 <h4 class="text-sm font-medium text-foreground">{{ t('repository.connectivity') }}</h4>
 </div>
 <div class="grid grid-cols-2 gap-3 text-sm">
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.reachable') }}</p>
 <p :class="['font-medium', selectedTestResult.details.connectivity.reachable ? 'text-emerald-600' : 'text-red-600']">
 {{ selectedTestResult.details.connectivity.reachable ? t('common.yes') : t('common.no') }}
 </p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.responseTime') }}</p>
 <p class="font-medium text-foreground">
 {{ selectedTestResult.details.connectivity.response_time || 0 }}ms
 </p>
 </div>
 <div v-if="selectedTestResult.details.connectivity.error" class="col-span-2">
 <p class="text-xs text-foreground-secondary">{{ t('common.error') }}</p>
 <p class="font-medium text-red-600">{{ selectedTestResult.details.connectivity.error }}</p>
 </div>
 </div>
 </div>

 <!-- Write Test -->
 <div v-if="selectedTestResult.details.write_test" class="bg-background-secondary rounded-lg p-4">
 <div class="flex items-center gap-2 mb-2">
 <PencilIcon class="w-5 h-5 text-blue-600 dark:text-blue-400" />
 <h4 class="text-sm font-medium text-foreground">{{ t('repository.writeTest') }}</h4>
 </div>
 <div class="grid grid-cols-3 gap-3 text-sm">
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.writable') }}</p>
 <p :class="['font-medium', selectedTestResult.details.write_test.writable ? 'text-emerald-600' : 'text-red-600']">
 {{ selectedTestResult.details.write_test.writable ? t('common.yes') : t('common.no') }}
 </p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.writeSpeed') }}</p>
 <p class="font-medium text-foreground">
 {{ selectedTestResult.details.write_test.write_speed || 0 }} MB/s
 </p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.readSpeed') }}</p>
 <p class="font-medium text-foreground">
 {{ selectedTestResult.details.write_test.read_speed || 0 }} MB/s
 </p>
 </div>
 <div v-if="selectedTestResult.details.write_test.error" class="col-span-3">
 <p class="text-xs text-foreground-secondary">{{ t('common.error') }}</p>
 <p class="font-medium text-red-600">{{ selectedTestResult.details.write_test.error }}</p>
 </div>
 </div>
 </div>

 <!-- Space Info -->
 <div v-if="selectedTestResult.details.space_info" class="bg-background-secondary rounded-lg p-4">
 <div class="flex items-center gap-2 mb-2">
 <CircleStackIcon class="w-5 h-5 text-purple-600 dark:text-purple-400" />
 <h4 class="text-sm font-medium text-foreground">{{ t('repository.storageInfo') }}</h4>
 </div>
 <div class="grid grid-cols-3 gap-3 text-sm mb-3">
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.total') }}</p>
 <p class="font-medium text-foreground">
 {{ formatBytes(selectedTestResult.details.space_info.total_bytes || 0) }}
 </p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.used') }}</p>
 <p class="font-medium text-foreground">
 {{ formatBytes(selectedTestResult.details.space_info.used_bytes || 0) }}
 </p>
 </div>
 <div>
 <p class="text-xs text-foreground-secondary">{{ t('repository.free') }}</p>
 <p class="font-medium text-foreground">
 {{ formatBytes(selectedTestResult.details.space_info.free_bytes || 0) }}
 </p>
 </div>
 </div>
 <!-- Progress bar -->
 <div class="w-full bg-slate-200 dark:bg-slate-600 dark:bg-slate-600 rounded-full h-2">
 <div
 class="bg-purple-600 h-2 rounded-full transition-all duration-500"
 :style="{ width: `${selectedTestResult.details.space_info.total_bytes ? ((selectedTestResult.details.space_info.used_bytes / selectedTestResult.details.space_info.total_bytes) * 100).toFixed(1) : 0}%` }"
 />
 </div>
 <p class="text-xs text-foreground-secondary text-center mt-1">
 {{ selectedTestResult.details.space_info.total_bytes ? ((selectedTestResult.details.space_info.used_bytes / selectedTestResult.details.space_info.total_bytes) * 100).toFixed(1) : 0 }}% {{ t('repository.used') }}
 </p>
 </div>
 </div>
 </div>
 ...
 <button @click="showTestResultModal = false" class="px-4 py-2 text-sm text-foreground-secondary border border-border rounded-lg hover:bg-hover/50">
 {{ t('common.close') }}
 </button>
 </div>
 </div>
